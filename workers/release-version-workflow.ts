...
import { getLanguages, getLatestVersion, getTranslationsByLocale, promoteVersion } from '~/lib/db.server';
import { gzipString } from '~/lib/gzip';

type Params = {}

export class ReleaseVersionWorkflow extends WorkflowEntrypoint<
	Env,
	Params
> {
  async run(
    ...
    const draftVersion = await step.do('get draft version', {
      retries: {
        limit: 3,
        delay: '2 seconds',
        backoff: 'exponential'
    ...
    const [defaultLanguage, restLanguages] = await step.do('get languages', {
      retries: {
        limit: 3,
        delay: '2 seconds',
        backoff: 'exponential'
    ...
    const defaultTranslations = await step.do('get default translations', {
      retries: {
        limit: 3,
        delay: '2 seconds',
        backoff: 'exponential'
    ...
    const restTranslations = await step.do('get rest translations', {
      retries: {
        limit: 3,
        delay: '3 seconds',
        backoff: 'exponential'
      ...
      console.log('[ReleaseVersionWorkflow] Getting the rest of the translations');
      const restTranslations = await Promise.all(restLanguages.map(async (language) => {
    ...
    const jsonFiles = await step.do('generate json files', {
      retries: {
        limit: 2,
        delay: '1 second',
        backoff: 'linear'
    ...
    await step.do('generate backup file', {
      retries: {
        limit: 5,
        delay: '3 seconds',
        backoff: 'exponential'
      ...
      await this.env.BACKUPS_BUCKET.put(`${draftVersion.id}/backup.gz`, compressed);
    });

    await step.do('promote draft version', {
      retries: {
        limit: 5,
        delay: '2 seconds',
        backoff: 'exponential'
