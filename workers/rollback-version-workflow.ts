...
export class RollbackVersionWorkflow extends WorkflowEntrypoint<
	Env,
	Params
> {
  async run(
    ...
    await step.do('check if version exists', {
      retries: {
        limit: 3,
        delay: '2 seconds',
        backoff: 'exponential'
    ...
    const backupData = await step.do('get backup data', {
      retries: {
        limit: 5,
        delay: '3 seconds',
        backoff: 'exponential'
      ...
      const file = await this.env.BACKUPS_BUCKET.get(`${versionId}/backup.gz`);
      if (!file) {
    ...
    await step.do('delete translations and languages', {
      retries: {
        limit: 3,
        delay: '2 seconds',
        backoff: 'exponential'
      ...
      console.log('[RollbackVersionWorkflow] Deleting translations and languages');
      await db.delete(translations);
      await db.delete(languages);
    });

    await step.do('insert languages from backup data', {
      retries: {
        limit: 3,
        delay: '2 seconds',
        backoff: 'exponential'
      ...
      console.log('[RollbackVersionWorkflow] Inserting languages from backup data');
    ...
    await step.do('insert translations from backup data', {
      retries: {
        limit: 5,
        delay: '3 seconds',
        backoff: 'exponential'
      ...
      console.log('[RollbackVersionWorkflow] Inserting translations from backup data');
      await Promise.all(backupData.map((values: any) => {    
        return db.insert(translations).values(values);
      }));
    });

    await step.do('promote archived version', {
      retries: {
        limit: 5,
        delay: '2 seconds',
        backoff: 'exponential'
